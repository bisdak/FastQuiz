...
def create_app(test_config=None):
    ...
    def add_question():
        ...
        if answer not in choices:
            return (
                jsonify({"error": "The correct answer must be one of the choices"}),
                400,
            )
    ...
    def get_question():
        ...
        question_doc = questions_collection.aggregate(
        ...
        return (
            jsonify({"question": question_data["question"], "choices": all_choices}),
            200,
        )
        ...
        try:
            ...
            if not all(col in df.columns for col in required_columns):
                return (
                    jsonify(
                        {
                            "error": f"Missing one or more required columns: {required_columns}"
                        }
                    ),
                    400,
                )
            ...
            for _, row in df.iterrows():
                question_data = {
                    ...
                    "choices": row["choices"].split(
                        ","
                    ),  # Assume choices are stored as comma-separated values
                    "level": row["level"],
            ...
            return (
                jsonify({"message": "File uploaded and data inserted successfully"}),
                201,
            )
